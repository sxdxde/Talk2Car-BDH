...
class TransVGDataset(data.Dataset):
    SUPPORTED_DATASETS = {
        ...
        'flickr': {
            'splits': ('train', 'val', 'test')},
        # Talk2Car port (our cross-architecture validation). Entries are stored
        # in the SAME (img_file, _, bbox_xywh, phrase, attri) 5-tuple layout as
        # the refcoco-family splits (see arch3/build_transvg_talk2car.py), so
        # pull_item's existing refcoco branch (xywh -> x1y1x2y2) applies as-is.
        # 'test' has no public GT (AIcrowd server closed 2020) -- val is the
        # eval split, matching the AttnGrounder phase.
        'talk2car': {
            'splits': ('train', 'val', 'test')}
    }
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', 
        ...
        elif self.dataset == 'talk2car':
            self.dataset_root = osp.join(self.data_root, 'talk2car')
            self.im_dir = osp.join(self.dataset_root, 'images')
        else:   ## refcoco, etc.
